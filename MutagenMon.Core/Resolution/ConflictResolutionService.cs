...
public sealed class ConflictResolutionService
{
    ...
    /// <summary>Copies the winning side over the other and
    ...
    /// split out from running the tool and propagating the result so a caller (the App
    ...
    /// <summary>Launches the configured visual merge tool on the two local copies.</summary>
    ...
    /// Checking only the alpha-side copy
    ...
    /// than moving on to the next one, matching the retry
    /// loop's behavior.</summary>
