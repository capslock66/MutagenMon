...
/// <summary>
/// Ports mutagenmonlib/remote/monitor.py: Monitor's polling loop, including
/// the per-session automatic-restart pass (FR-13; ports restart_mutagen()).
...
public sealed class SessionMonitorService : BackgroundService
{
    ...
    /// <summary>Ports mutagenmonlib/remote/monitor.py: restart_mutagen()
    /// (FR-13). For each known session, re-derives which (if any) of the
    ...
    /// <summary>Ports mutagenmonlib/remote/mutagen.py: restart_session()
    /// (FR-13.5) — terminate then recreate from the session's original
    /// definition, each step independently tolerant of the other's
