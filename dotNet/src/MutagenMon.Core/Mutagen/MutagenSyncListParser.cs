...
/// <summary>
/// Pure port of mutagenmonlib/remote/mutagen.py: get_session_status() (plus
/// the text cleanup mutagen_sync_list() did before handing off to it). Takes
/// zero dependencies on a real `mutagen` process — see NFR-11 — so the
/// classification pipeline it feeds is fully unit-testable.
///
...
/// legacy Python raises a KeyError there, which its caller silently swallows
/// — meaning ALL status polling for EVERY session silently stops every cycle
/// as long as the stray session exists. This port instead just carries the
...
public static partial class MutagenSyncListParser
{
    ...
    /// <summary>Ports the text cleanup half of mutagen_sync_list() (the timestamp
    /// prefix it also added is a display-only concern for the status view, FR-8 —
    ...
    /// <summary>Ports mutagenmonlib/local/lib.py: get_matching_open_parenth().
    /// Finds, scanning backwards from just before <paramref name="closeIndex"/>
