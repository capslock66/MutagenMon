...
/// Ports mutagenmonlib/remote/{resolve,ssh}.py's process invocations, but
/// passes arguments via <see cref="ProcessStartInfo.ArgumentList"/> instead
/// of building a shell command line — the legacy's <c>remote_escape</c>
/// (manually escaping spaces/parens/ampersands for a shell string) is a
...
public sealed class ConflictFileClient : IConflictFileClient
{
    ...
    /// <summary>Ports mutagenmonlib/local/file.py: dir_and_name() — joins a
