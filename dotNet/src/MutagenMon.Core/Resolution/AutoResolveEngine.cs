...
/// <summary>
/// Ports mutagenmonlib/remote/monitor.py: auto_resolve()/
/// auto_resolve_single()/clean_auto_resolve_history() (FR-10). Run once per
...
public sealed class AutoResolveEngine
{
    ...
    /// <summary>Ports auto_resolve(): returns a new conflicts dictionary with
    ...
    /// <summary>Ports clean_auto_resolve_history().</summary>
    ...
    /// <summary>Ports auto_resolve_single(): first matching rule (unanchored
    /// regex search against the file name, mirroring the legacy's
    /// <c>re.search</c>) wins and is applied immediately, no user
    /// interaction.</summary>
