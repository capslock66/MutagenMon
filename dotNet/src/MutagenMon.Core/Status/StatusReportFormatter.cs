...
/// <summary>
/// Ports mutagenmonlib/wx/icon.py: TaskBarIcon.get_nice_log() / get_conflict_names()
/// (FR-8.1/FR-8.2) — builds the status view's body from the already-structured
/// <see cref="ParsedSessionStatus"/>/<see cref="ConflictRecord"/> data (rather than
/// regex-scrubbing the raw CLI log, as the legacy did) so identifiers are simply
...
public static class StatusReportFormatter
{
    ...
    /// <summary>Ports get_conflict_names(): true if at least one conflict is
